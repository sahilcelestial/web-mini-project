...
function get_posts(){
	...
	if(isset($_GET['post_id'])){
		...
		$get_con="select * from users where user_email='$user_con'";

		$run_con=mysqli_query($con,$get_con);
		$row_con=mysqli_fetch_array($run_con);

		$user_con_ic=$row_con['user_id'];
		$user_con_name=$row_con['user_name'];
		...
		if(isset($_POST['reply'])){
			...
			if($comment==""){
				...
				$insert="insert into comments (post_id,user_id,comment,comment_author,date) values('$post_id','$user_id','$comment','$user_con_name',NOW())";

				$run=mysqli_query($con,$insert);

				echo"<script>alert('Your Comment Aided!')</script>";
				echo"<script>window.open('single.php?post_id=$post_id','_self')</script>";
			...
			<div class='row'>
				...
				<div class='col-sm-6'>
					<div class='row' id='find_people'>
						...
						<div class='col-sm--6'>
							<a style='text-decoration:none; cursor: pointer;color:#3897f0 ; 'href='user_profile.phph?u_id=$user_id'> <strong><h2>$f_name $l_name</h2></strong> </a>
						</div>
						<div class='col-sm-3'>
						</div>
					</div>
			...
			<div class='row'>
				...
				<div class='col-sm-6'>
					<div class='row' id='find_people'>
						...
						<div class='col-sm--6'>
							<a style='text-decoration:none; cursor: pointer;color:#3897f0 ; 'href='user_profile.php?u_id=$user_id'> <strong><h2>$f_name $l_name</h2></strong> </a>
						</div>
						<div class='col-sm-3'>
						</div>
					</div>
